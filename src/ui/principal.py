...
from src.servicos.diagnosticos import registrar_log as registrar_log_arquivo
from src.servicos.historico_empresas import (
    contagem_busca as contagem_historico_busca,
)
from src.servicos.historico_empresas import (
    estatisticas,
...
class JanelaPrincipal(QMainWindow):
    def __init__(self, cliente: ClienteWebRP, email: str, webrp_url: str) -> None:
        ...
        indice_politica = self.combo_continuacao.findData(filtros_salvos.politica_continuacao)
        self.combo_continuacao.setCurrentIndex(max(0, indice_politica))
        ...
        QTimer.singleShot(1800, self._verificar_atualizacao_automatica)
    ...
    def _montar_cabecalho(self) -> QFrame:
        ...
        self.botao_atualizacao = QPushButton(f"v{VERSAO}")
        self.botao_atualizacao.setObjectName("sair")
        self.botao_atualizacao.setToolTip("Verificar se existe uma versão nova")
        self.botao_atualizacao.clicked.connect(self._verificar_atualizacao_manual)

        direita = QHBoxLayout()
        direita.setSpacing(10)
        direita.addWidget(self.botao_atualizacao)
    ...
    def _montar_busca(self) -> QFrame:
        ...
        self.check_continuar = criar_opcao(
            "Continuar busca",
            "Usa o histórico local para escolher quais empresas devem ser puladas.",
        )
        self.combo_continuacao = QComboBox()
        self.combo_continuacao.addItem("Pular todas processadas", "todas")
        self.combo_continuacao.addItem("Pular somente importadas", "importadas")
        self.combo_continuacao.addItem("Refazer após 30 dias", "recentes30")
        self.combo_continuacao.setToolTip(
            "Define quais empresas do histórico não devem aparecer novamente nesta busca."
        )
        self.combo_continuacao.currentIndexChanged.connect(self._preferencias_alteradas)
        ...
        linha_controles.setSpacing(10)
        linha_controles.addWidget(self.check_continuar)
        linha_controles.addWidget(self.combo_continuacao)
        linha_controles.addWidget(self.check_cruzar)
        linha_controles.addWidget(self.check_variada_auto)
    ...
    def _montar_resultados(self) -> QFrame:
        ...
        self.botao_descartar = criar_botao("Descartar marcados", "secundario", compacto=True)
        self.botao_descartar.setEnabled(False)
        self.botao_descartar.clicked.connect(self._descartar_selecionados)
        ...
        barra_layout.addWidget(
            criar_grupo_botoes(
                self.botao_selecionar,
                self.botao_desmarcar,
                self.botao_descartar,
                self.botao_importar,
            )
        )
    ...
    def _preferencias_alteradas(self) -> None:
        salvar_filtros(
            FiltrosSalvos(
                ...
                politica_continuacao=str(self.combo_continuacao.currentData() or "todas"),
    ...
    def _atualizar_status_busca(self) -> None:
        ...
        partes: list[str] = []
        total = max(
            contagem_progresso(consulta, local),
            contagem_historico_busca(consulta, local),
        )
        if total:
            partes.append(f"{total} lugar(es) já processado(s) nesta busca.")
        totais_historico = estatisticas()
        if totais_historico["total"]:
            partes.append(
                f"Histórico: {totais_historico['importada']} importada(s), "
                f"{totais_historico['extraida']} aguardando revisão."
            )
    ...
    def _reiniciar_progresso_busca(self) -> None:
        ...
        limpar_historico_busca(consulta, local)
    ...
    def _iniciar_extracao(self) -> None:
        ...
        self.worker_extracao = ExtracaoWorker(
            ...
            str(self.combo_continuacao.currentData() or "todas"),
            self.cliente,
            self,
        )
        ...
        self.worker_extracao.evento.connect(self._evento_processado)
    ...
    def _evento_processado(self, evento) -> None:
        nome = evento.lugar.nome if evento.lugar is not None else "link do Maps"
        if evento.status in {"duplicada", "falha"}:
            detalhe = f" — {evento.motivo}" if evento.motivo else ""
            self.label_feedback.setText(f"{nome}: {evento.status}{detalhe}")
    ...
    def _formatar_resumo(self, resultado: ResultadoExtracao) -> str:
        ...
        if resultado.alertas_duplicidade:
            partes.append(f"{resultado.alertas_duplicidade} possível(is) duplicata(s) para revisão")
        return " — ".join(partes)
    ...
    def _atualizar_tabela(self) -> None:
        ...
        if total == 0:
            ...
            if self._ultimo_resultado and (
                ...
                self.botao_descartar.setEnabled(habilitar)
                return
    ...
    def _descartar_selecionados(self) -> None:
        ...
        resposta = QMessageBox.question(
            self,
            "Descartar empresas",
            f"Marcar {len(selecionados)} empresa(s) como descartada(s) e removê-las dos resultados?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        ...
        consulta = self.campo_consulta.text().strip()
        local = self.campo_local.text().strip()
        selecionados_em_memoria = {id(lugar) for lugar in selecionados}
        for lugar in selecionados:
            registrar_lugar(lugar, consulta, local, "descartada", "Descartada manualmente")
        self.lugares = [
            lugar for lugar in self.lugares if id(lugar) not in selecionados_em_memoria
        ]
        self._registrar_log(f"{len(selecionados)} empresa(s) descartada(s).")
        self._atualizar_tabela()
        self._atualizar_status_busca()
    ...
    def _importar(self) -> None:
        ...
        self._lugares_importando = list(selecionados)
        self.worker_importacao = ImportacaoWorker(
            self.cliente,
            selecionados,
            self.campo_consulta.text().strip(),
            self.campo_local.text().strip(),
            self,
        )
    ...
    def _importacao_concluida(self, mensagem: str, resultados: list) -> None:
        ...
        concluidos = {
            id(lugar)
            for lugar, resultado in zip(self._lugares_importando, resultados)
            if resultado.sucesso or resultado.status == 409
        }
        if concluidos:
            self.lugares = [lugar for lugar in self.lugares if id(lugar) not in concluidos]
        self._lugares_importando = []
        self._atualizar_tabela()
    ...
    def _abrir_historico(self) -> None:
        DialogoHistorico(self).exec()
        self._atualizar_status_busca()

    def _verificar_atualizacao_automatica(self) -> None:
    ...
    def _iniciar_verificacao_atualizacao(self, manual: bool) -> None:
        if self.worker_atualizacao and self.worker_atualizacao.isRunning():
            return
        self._atualizacao_manual = manual
        self.botao_atualizacao.setEnabled(False)
        self.botao_atualizacao.setText("Verificando…")
        self.worker_atualizacao = AtualizacaoWorker(self.webrp_url, self)
        self.worker_atualizacao.concluido.connect(self._atualizacao_concluida)
        self.worker_atualizacao.erro.connect(self._atualizacao_erro)
        self.worker_atualizacao.start()
    ...
    def _atualizacao_erro(self, mensagem: str) -> None:
        ...
        if self._atualizacao_manual:
            ...
            caixa.setWindowTitle("Atualização indisponível")
            caixa.setText(mensagem)
            caixa.setInformativeText(
                "Você pode abrir a página de versões e conferir o download manualmente."
            )
            abrir = caixa.addButton("Abrir versões", QMessageBox.ButtonRole.AcceptRole)
            caixa.addButton(QMessageBox.StandardButton.Cancel)
            caixa.exec()
            if caixa.clickedButton() is abrir:
                QDesktopServices.openUrl(
                    QUrl("https://github.com/kaueajure/extrator/releases/latest")
                )
