...
class ExtracaoWorker(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            ignorar = (
                ler_urls_processadas(self.consulta, self.local)
                if self.continuar_busca
                else set()
            )

            def ja_existe(lugar: LugarExtraido) -> bool:
                return indice.contem(lugar) if indice else False

            def ja_existe_url(url: str) -> bool:
                return indice.contem_url(url) if indice else False
...
class ImportacaoWorker(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            resultados = self.cliente.importar_lugares(self.lugares, on_progresso)
