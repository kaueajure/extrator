...
from src.servicos.identidade import Correspondencia
...
EventoCallback = Callable[[EventoProcessamento], None]
DuplicidadeLugarCallback = Callable[[LugarExtraido], Correspondencia]
DuplicidadeUrlCallback = Callable[[str], Correspondencia]
...
def _extrair_detalhes(
    ...
    for tentativa in range(1, 4):
        try:
            ...
            if tentativa >= 3:
                raise
            _log(callback, f"Página lenta; nova tentativa {tentativa + 1}/3…")
            page.wait_for_timeout(800 * tentativa)
...
def extrair_lugares(
    ...
    ja_existe_webrp: DuplicidadeLugarCallback | None = None,
    ja_existe_url: DuplicidadeUrlCallback | None = None,
        ...
        try:
            ...
            for tentativa in range(1, 4):
                try:
                    ...
                    if tentativa >= 3:
                        raise
                    _log(callback, f"Maps demorou para responder; nova tentativa {tentativa + 1}/3…")
            ...
            for indice, link in enumerate(links, start=1):
                ...
                correspondencia_url = ja_existe_url(url_norm) if ja_existe_url else Correspondencia(False)
                if correspondencia_url.duplicado:
                    ignorados_webrp += 1
                    urls_vistas.add(url_norm)
                    motivo = correspondencia_url.motivo or "link conhecido"
                    _log(callback, f"Ignorado (já no WebRP) — {motivo}.")
                    if on_evento:
                        on_evento(EventoProcessamento(url_norm, "duplicada", motivo=motivo))
                    continue
                try:
                    lugar = _extrair_detalhes(page, link, callback, visivel)
                    urls_vistas.add(_normalizar_url_lugar(link))
                    correspondencia = (
                        ja_existe_webrp(lugar) if ja_existe_webrp else Correspondencia(False)
                    )
                    if correspondencia.duplicado:
                        ignorados_webrp += 1
                        motivo = correspondencia.motivo or "correspondência exata"
                        _log(callback, f"Ignorado (já no WebRP): {lugar.nome} — {motivo}.")
                        if on_evento:
                            on_evento(EventoProcessamento(url_norm, "duplicada", lugar, motivo))
                        continue
                    if correspondencia.apenas_alerta:
                        alertas_duplicidade += 1
                        _log(
                            callback,
                            f"Possível duplicata: {lugar.nome} — {correspondencia.motivo}; "
                            "mantida para revisão.",
                        )
                    lugares.append(lugar)
                    if on_evento:
                        on_evento(EventoProcessamento(url_norm, "extraida", lugar))
                    if on_lugar:
                        on_lugar(lugar)
                    ...
                    if on_evento:
                        on_evento(EventoProcessamento(url_norm, "falha", motivo=str(erro)))
    ...
    return ResultadoExtracao(
        ...
        ignorados_webrp=ignorados_webrp,
        ignorados_continuacao=ignorados_continuacao,
        alertas_duplicidade=alertas_duplicidade,
    )
